...
const Pagination = ({ pageLength, page:{currentPage, onPageChange} }: PaginationProps): JSX.Element => {
    ...
    {
        mid[4]!== pageLength &&  <Button mode='secondary-outline' onClick={()=>onPageChange(pageLength)} key={pageLength} borderColor={pageLength===currentPage ? 'pri.01' : 'nl.05'} color={pageLength===currentPage ? 'pri.01' : 'nl.02'} mx={1}  fontSize={'sm'} borderRadius={'base'}>{pageLength}</Button>
    }
