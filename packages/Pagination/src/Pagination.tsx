...
const Pagination = ({ pageLength, page: { currentPage, onPageChange } }: PaginationProps): JSX.Element => {
    ...
    return <Flex>
        <IconButton variant='secondary-outline' size='sm' isDisabled={currentPage === 1} onClick={() => onPageChange(Math.max(1, currentPage - 1))} aria-label="pagination" sx={styles} icon={<Icon as={FaAngleLeft} />} />
        {
        ...
        <IconButton variant='secondary-outline' size='sm' aria-label="pagination" isDisabled={currentPage === pageLength} onClick={() => onPageChange(Math.min(pageLength, currentPage + 1))} sx={styles} icon={<Icon as={FaAngleRight} />} />
    </Flex>
}
